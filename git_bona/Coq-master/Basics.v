...
Definition invnybble (nb : nybble) : nybble :=
    match nb with
    | bits b1 b2 b3 b4 => bits (invbit b1) (invbit b2) (invbit b3) (invbit b4)
    end.

Compute (invbit B0).
Compute (invnybble (bits B0 B1 B0 B1)).
Compute (all_zero (bits B1 B0 B1 B0)).
(* ===> false : bool *)
Compute (all_zero (bits B0 B0 B0 B0)).
(* ===> true : bool *)

Theorem inv_involutive : forall b, invbit (invbit b) = b.
...
Theorem invnybble_involutive : forall b, invnybble (invnybble b) = b.
Proof. intros x. destruct x. simpl. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive. rewrite inv_involutive. reflexivity. Qed.
(* ================================================================= *)
(** ** Modules *)
...
Proof.
  ...
  + destruct c. 
    - reflexivity.
    - inversion H. (* ou simpl in H. apply H. *)
  + destruct c.
    - reflexivity.
    - inversion H.
Qed.
...
Proof.
  ...
  + destruct c.
    - reflexivity.
    - simpl in H. rewrite H. reflexivity. (*ou inversion H.*)
  + destruct c.
    - reflexivity.
...
(*
 ...
 intros - introduz variaveis
rewrite - substitui uma variavel (da pra usar H in H2, ai troca em cima)
induction n - divide em subtermos e cria Hipotese apos provada o primeiro subtermo (as [|n H]
destruct n - divide em subtermos lists.v, destroi lista ou pares em 2 elementos
assert - tb divide em subtermos assert (H: n+m = m+n) - vc cria um subtermo, e se provar que ele ta certo vc pode usar
rewrite plus_n_Sm  s ( s p + a) = sp + sa
rewrite plus_comm - m+ n =n+m  →  n+m=n+m
rewrite app_assoc - (rev t ++ [h]) ++ [ ] =  rev t ++ [h] ++ [ ]
rewrite mullt_n_O e plus_n_O - 0 + n = n
rewrite mult_assoc - m * ( n * p ) = m* n*p
rewrite mult_comm - m *  n *  m = m* ( n*m )
apply - rewrite + reflexivity
apply f_equal - S n’ = S m’ → n’ = m’
introduction - introduz uma lista dividida??? (intro + destruct/induction???????) 
symmetry - troca os lados, pode usar in pra aplicar em cima (in H)
injection H as H1 H2 - (usado em listas) divide um H em H1 e H2 (nem sempre, injection H as Hnm)
discriminate se uma hipótese é falsa então todo o resto é verdade Qed. (discriminate H)
simpl in H - simplifica na hipótese
unfold - usa uma definição que voce criou? (in) (unfold not = ~ P vai para P -> false)
split - divide /\ e \/
left - A \/ B == A
right - A \/ B == B
exfalso - true = false → false
inversion = usado no IndProp, só Deus sabe pra que serve
