...
Proof.
  intros. apply evSS_ev in H. apply evSS_ev in H. apply H.
Qed.
(** [] *)
...
Proof.
  intros a b c H1 H2. apply ev_ev__ev with (n:=a) (m:=b) in H1.
Abort.

Theorem ev_plus_plus : forall n m p,
  even (n+m) -> even (n+p) -> even (m+p).
Proof.
  intros a b c H1 H2. apply ev_sum with (a+b) (a+c) in H1.
Admitted.
(** [] *)
...
Proof.
  intros. induction H0.
  - apply H.
  - rewrite IHle. apply le_S. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite IHle. apply le_S. reflexivity. (* apply le_S. apply IHle *)
Qed.
...
Proof.
  intros. induction b.
  - SearchRewrite (_ + 0). rewrite <- plus_n_O. apply le_n.
  - SearchRewrite (_ + S _). rewrite <- plus_n_Sm. apply le_S. apply IHb.
Qed.
...
Inductive reg_exp {T : Type} : Type :=
  | EmptySet (*vazio*)
  | EmptyStr (*apenas com a palavra vazia*)
  | Char (t : T) (* elemento *)
  | App (r1 r2 : reg_exp) (*concatenação*)
  | Union (r1 r2 : reg_exp) (*OU*)
  | Star (r : reg_exp). (*fecho de Klein, aquele asterisco * => diversas ocorrencias, podendo ter o vazio*)
...
Proof.
  apply (MApp [1] _ [2] _). (* = apply MApp with (s1:=[1]) (s2:=[2])*)
  - apply MChar.
  - apply MChar.
Qed.
