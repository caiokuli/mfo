...
Proof.
  intros n m. induction n as [|n' Hn].
    - induction m as  [|m' Hm].
      + simpl. reflexivity.
      + simpl. rewrite <- Hm. simpl. reflexivity.
  - induction m as  [|m' Hm].
      + simpl. rewrite Hn. simpl. reflexivity.
      + simpl. rewrite Hn. rewrite plus_n_Sm. simpl. reflexivity.
Qed. 
...
Proof.
  intro. induction n. 
    - simpl. reflexivity.
    - simpl. rewrite  IHn. SearchRewrite (_ + S _). rewrite plus_n_Sm. reflexivity.
Qed. 
(** [] *)
...
Proof.
  intro. induction n as [|n' IH].
    - reflexivity.
    - rewrite evenb_SS. rewrite IH. SearchRewrite (negb (negb _)).
...
Theorem evenb_S' : forall n : nat,
 ...
 Proof.
  intro. induction n as [|n' IH].
  - reflexivity. 
  - assert (eSS: evenb (S (S n')) = evenb (n')). {reflexivity. }
    rewrite eSS. rewrite IH.
    rewrite negb_involutive. reflexivity. 
Qed. 
(** [] *)
...
Proof.
  intros n m p. induction n as [|n' Hn].
    - simpl. reflexivity.
    - simpl.  rewrite Hn. SearchRewrite (_  + S _). rewrite plus_n_Sm. reflexivity.
Qed.
...
Theorem mult_comm : forall m n : nat,
  m * n = n * m.
Proof.
  intros m n. induction n as [|n' H].
    -  simpl. SearchRewrite (_ * 0). rewrite mult_0_r. reflexivity.
...
Proof.
  intros n. simpl. SearchRewrite (_ + 0). rewrite plus_n_O. reflexivity.
Qed.
