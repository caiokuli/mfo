...
Proof.
  ...
  - destruct b as [|b'].
    ...
    + destruct a as [|a'].
      ...
      * discriminate c.
Qed.
(*da pra usar comutatividade da soma, depois usa discriminate /\*)
(** [] *)
...
Fact not_implies_our_not : forall (P:Prop),
  ~ P -> (forall (Q:Prop), P -> Q).
Proof.
  intros P H1 Q H2. unfold not in H1. apply H1 in H2. destruct H2.
Qed.
...
Theorem contrapositive : forall (P Q : Prop),
  (P -> Q) -> (~Q -> ~P).
Proof.
  intros P Q A B C. destruct B. apply A. apply C. Qed.
(** [] *)
...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P Q. apply Q. destruct Q. apply H. Qed.
(** [] *)
...
Theorem iff_refl : forall P : Prop,
  P <-> P.
Proof.
  intros P. reflexivity. Qed.

Theorem iff_trans' : forall P Q R : Prop,
  (P <-> Q) -> (Q <-> R) -> (P <-> R).
Proof.
  intros P Q R [H1 H2] [H3 H4]. split.
  - intro H5. apply H1 in H5.
  apply H3 in H5. apply H5.
  - intro H5. apply H4 in H5.
...
Proof.
  intros P Q R [H1 H2].
  apply iff_trans. split. apply H1. apply H2.
Qed. 
...
Proof.
  (* WORKED IN CLASS *)
  intros n H. destruct H as [m Hm]. (* note implicit [destruct] here *)
  exists (2 + m).
  apply Hm.  Qed.
...
Proof.
  intros X P H. unfold not. intro H1. destruct H1 as [x H2]. apply H2 in H.
destruct H. Qed.

Theorem dist_not_exists' : forall (X:Type) (P : X -> Prop),
  (forall x, P x) -> ~ (exists x, ~ P x).
Proof.
  intros X P H [x H1]. unfold not in H1. apply H1 in H. apply H. Qed.
...
Proof.
   intros X P Q. split.
    - intros H1. destruct H1 as [x [HP | HQ]].
      + left. exists x. apply HP.
      + right. exists x. apply HQ.
    - intros H1. destruct H1 as [HP | HQ].
