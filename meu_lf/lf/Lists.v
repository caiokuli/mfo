...
Theorem surjective_pairing_stuck : forall (p : natprod),
  p = (fst p, snd p).
Proof.
  simpl. intro p. destruct p as [n m]. simpl. reflexivity.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l with 
  | nil => nil
  | n :: r => match (evenb n) with
              | true => oddmembers r
              | false => n::(oddmembers r)
                end
  end.
...
Fixpoint countoddmembers (l:natlist) : nat :=
  ...
  | n::r => match (oddb n) with
             | true => S (countoddmembers r)
             | false => countoddmembers r
              end
  end.
